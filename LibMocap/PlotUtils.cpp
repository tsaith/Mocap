...
namespace plot_utils {
    ...
    cv::Mat CombineImages3x1(cv::Mat& Img1, cv::Mat& Img2, cv::Mat& Img3, cv::Size SizeOut) {
        ...
        // Resize the images to the final size / 3

        int patchWidth = static_cast<int>(SizeOut.width / 3);
        int patchHeight = static_cast<int>(SizeOut.height / 3);
        cv::Size patchSize = cv::Size(patchWidth, patchHeight);

        cv::Mat resized1, resized2, resized3;
        cv::resize(Img1, resized1, patchSize);
        cv::resize(Img2, resized2, patchSize);
        cv::resize(Img3, resized3, patchSize);

        // Create the final image
        cv::Mat imageOut(SizeOut, Img1.type());

        // Insert the images
        resized1.copyTo(imageOut(cv::Rect(0, 0, patchWidth, patchHeight)));
        resized2.copyTo(imageOut(cv::Rect(patchWidth, 0, patchWidth, patchHeight)));
        resized3.copyTo(imageOut(cv::Rect(2*patchWidth, 0, patchWidth, patchHeight)));
